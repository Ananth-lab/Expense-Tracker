...
window.addEventListener("DOMContentLoaded", async () => {
    try {
        ...
        const expenses = await axios.get(`http://35.76.210.189/expense/getExpense?page=${page} &perpage=${perPageSelected}`, { headers: { "authorization": token } });
...
function deleteExpense(id, dltBtn) {
    if (confirm(`This action cannot to undone. are you sure?`)) {
        axios.delete(`http://35.76.210.189/expense/deleteExpense/${id}`, { headers: { "authorization": token } })
            .then(() => {
...
subBtn.addEventListener("click", (e) => {
    ...
    axios.post("http://35.76.210.189/expense/addExpense", expense, { headers: { "authorization": token } })
        .then(res => {
...
premiumBtn.addEventListener("click", async (e) => {
    const response = await axios.get("http://35.76.210.189/get-premium/purchase-premium", { headers: { "authorization": token } });
    var options = {
        "key": response.data.key_id,
        "order_id": response.data.order.id,
        "handler": async function (response) {
            const result = await axios.post("http://35.76.210.189/get-premium/update-transaction-status", {
    ...
    rzrp1.on("payment.failed", () => {
        axios.post("http://35.76.210.189/get-premium/update-transaction-status", { order_id: response.data.order.id }, { headers: { "authorization": token } })
...
axios.get("http://35.76.210.189/premium/show-leaderboard", { headers: { "authorization": token } })
    .then(res => {
...
downloadReport.addEventListener("click", (e) => {
    e.preventDefault();
    axios.get("http://35.76.210.189/user/download-report", { headers: { "authorization": token } })
        .then((res) => {
...
axios.get("http://35.76.210.189/premium/show-file-audit", { headers: { "authorization": token } })
    .then(res => {
